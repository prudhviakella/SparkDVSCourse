...
object ShuffleBased {
  ...
  def main(args: Array[String]) {
    ...
    //job1&job2:Stage0
    val salesRDD = sc.textFile("/home/cloudera/projects/spark-core/src/main/resources/sales.csv",3)
    ...
    //job0&job2:Stage1
    val customerRDD = sc.textFile("/home/cloudera/projects/spark-core/src/main/resources/customers.csv",2)
    ...
    val Innerjoin = CogroupRDD.filter(element => element._2._1.size !=0 && element._2._2.size != 0)
    ...
    val action3 = Innerjoin.flatMap(element => {
      ...
      SalesRecordRDD.map(salesRecord=>(CustomerName,salesRecord.itemId,salesRecord.itemValue)) // This returns the List of results so flattening it
